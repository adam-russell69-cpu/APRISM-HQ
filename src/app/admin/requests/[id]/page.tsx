...
import { ChevronLeft, Clock3, MapPin, Wrench } from "lucide-react";
import { notFound } from "next/navigation";
import { updateServiceRequestStatus } from "../../actions";
import { createWorkOrderFromRequest } from "../../work-orders/actions";
...
export default async function RequestDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!request) notFound();
  const [propertyResult, workOrderResult] = await Promise.all([
    supabase.from("properties").select("id, name, address_line_1, city, state").eq("id", request.property_id).maybeSingle(),
    supabase.from("work_orders").select("id, status, assigned_to, created_at").eq("service_request_id", request.id).maybeSingle(),
  ]);
  ...
  return <main className="mx-auto max-w-6xl px-4 py-7 sm:px-6 lg:px-8 lg:py-9">
    ...
    <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_320px]">
      <section className="space-y-5">
        <div className="border border-black/10 bg-white p-5 sm:p-7"><p className="text-[0.65rem] font-semibold uppercase tracking-[0.14em] text-[#87682f]">Request detail</p><h2 className="mt-2 font-serif text-3xl">Scope and context</h2><p className="mt-5 whitespace-pre-wrap text-sm leading-7 text-black/62">{request.description}</p><dl className="mt-7 grid gap-4 border-t border-black/10 pt-5 sm:grid-cols-3"><div><dt className="text-xs font-semibold text-black/35">Category</dt><dd className="mt-1 text-sm text-black/68">{request.category}</dd></div><div><dt className="text-xs font-semibold text-black/35">Preferred timing</dt><dd className="mt-1 text-sm text-black/68">{request.preferred_timing || "Not specified"}</dd></div><div><dt className="text-xs font-semibold text-black/35">Last updated</dt><dd className="mt-1 text-sm text-black/68">{dateFormatter.format(new Date(request.updated_at))}</dd></div></dl></div>
        <div className="border border-[#94733a]/25 bg-[#fbf8ef] p-5 sm:p-6"><div className="flex items-start gap-3"><Wrench aria-hidden="true" className="mt-1 size-5 text-[#8b692f]" /><div className="flex-1"><p className="text-[0.6rem] font-semibold uppercase tracking-[0.14em] text-[#80612d]">V1-A field workflow</p>{workOrder ? <><h2 className="mt-1 font-serif text-2xl">Work order is active</h2><p className="mt-2 text-sm leading-6 text-black/52">This request is already connected to the field-service record. Open it to start or continue a service visit.</p><Link href={`/admin/work-orders/${workOrder.id}`} className="mt-5 inline-flex min-h-12 items-center justify-center bg-[#1b201e] px-5 text-xs font-semibold uppercase tracking-[0.12em] text-white">Open work order</Link></> : <><h2 className="mt-1 font-serif text-2xl">Create the field work order</h2><p className="mt-2 text-sm leading-6 text-black/52">This converts the client request into a trackable APRISM job with visits, timestamps, notes, and a permanent property record.</p><form action={createWorkOrderFromRequest} className="mt-5"><input type="hidden" name="requestId" value={request.id} /><button className="inline-flex min-h-12 items-center justify-center bg-[#1b201e] px-5 text-xs font-semibold uppercase tracking-[0.12em] text-white">Create work order</button></form></>}</div></div></div>
      </section>
