...
export default async function WorkOrderFieldPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: workOrder } = await supabase
    .from("work_orders")
    .select("id, property_id, service_request_id, title, description, status, priority, source, scheduled_at, created_at, completed_at")
    .eq("id", id)
    .maybeSingle();
  ...
  const [propertyResult, visitsResult, activityResult] = await Promise.all([
    workOrder.property_id
      ? supabase.from("properties").select("id, name, address_line_1, city, state, postal_code, sensitive_access_notes").eq("id", workOrder.property_id).maybeSingle()
    ...
    <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_330px]">
      <section className="space-y-5">
        <div className="border border-black/10 bg-white p-5 sm:p-6">
          <div className="flex items-start gap-3"><Wrench aria-hidden="true" className="mt-1 size-5 text-[#8d6b32]" /><div><p className="text-[0.58rem] font-semibold uppercase tracking-[0.14em] text-black/40">Job scope</p><p className="mt-2 whitespace-pre-wrap text-sm leading-7 text-black/65">{workOrder.description || "No additional description was provided."}</p></div></div>
        </div>

        {activeVisit ? <form action={endVisit} className="border-2 border-[#8c6c33]/35 bg-[#fbf8ef] p-5 sm:p-6">
          ...
          <div className="mt-6 grid gap-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2"><label className="text-xs font-semibold text-black/55">Visit outcome<select name="outcome" required defaultValue="complete" className="mt-2 min-h-12 w-full border border-black/15 bg-white px-3 text-sm"><option value="complete">Job complete</option><option value="return_required">Return visit required</option></select></label><label className="text-xs font-semibold text-black/55">Return reason<select name="returnReason" defaultValue="waiting_parts" className="mt-2 min-h-12 w-full border border-black/15 bg-white px-3 text-sm"><option value="waiting_parts">Waiting for parts</option><option value="awaiting_approval">Awaiting approval</option><option value="additional_diagnosis">Additional diagnosis</option><option value="scheduled_return">Scheduled return</option><option value="other">Other</option></select></label></div>
          </div>
