...
import { Building2, ChevronLeft, FileText, MailPlus, MapPin, ReceiptText, Users, Wrench } from "lucide-react";
import { addClientAccountMember, inviteClientAccountMember, updateClientAccount, updateClientAccountMember } from "../actions";
  ...
  return <main className="mx-auto max-w-[1500px] px-4 py-7 sm:px-6 lg:px-8 lg:py-9">
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
      ...
      <section className="border border-black/10 bg-white">
        <div className="border-b border-black/10 p-5 sm:p-6"><p className="text-[0.65rem] font-semibold uppercase tracking-[0.14em] text-[#87682f]">Portal authorization</p><h2 className="mt-2 font-serif text-3xl">Account members</h2><p className="mt-3 text-sm leading-6 text-black/48">Invite a new portal user by email or link an existing Supabase Auth user. Access is controlled by the membership record, not by profile metadata.</p></div>
        ...
        <form action={inviteClientAccountMember} className="border-t border-black/10 bg-[#f3efe5] p-5 sm:p-6"><input type="hidden" name="account_id" value={account.id} /><div className="flex items-start gap-3"><MailPlus aria-hidden="true" className="mt-1 size-5 text-[#87682f]" /><div><h3 className="font-serif text-2xl">Invite new portal user</h3><p className="mt-2 text-xs leading-5 text-black/45">Supabase creates the Auth user, APRISM binds the returned UUID to this account, then the invitee securely sets a password.</p></div></div><div className="mt-4 grid gap-4 md:grid-cols-[0.8fr_1fr_0.45fr_auto] md:items-end"><label className="text-xs font-semibold text-black/55">Name<input name="full_name" maxLength={160} placeholder="Trevor" className={inputClass} /></label><label className="text-xs font-semibold text-black/55">Email<input name="email" type="email" required maxLength={254} placeholder="name@example.com" className={inputClass} /></label><label className="text-xs font-semibold text-black/55">Role<select name="role" defaultValue="manager" className={inputClass}><option value="owner">Owner</option><option value="manager">Manager</option><option value="billing">Billing</option><option value="member">Member</option></select></label><button type="submit" className="min-h-11 bg-[#87682f] px-5 text-xs font-semibold text-white">Send Invite</button></div></form>
