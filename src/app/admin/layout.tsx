...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-[#eeece6] text-[#171a19]">
    <header className="print-hidden border-b border-white/10 bg-[#0d100f] text-white">
      <div className="mx-auto flex min-h-20 max-w-[1560px] items-center justify-between gap-5 px-5 sm:px-8 lg:px-10">
        ...
        <nav aria-label="Admin navigation" className="flex items-center gap-3 sm:gap-5">
          <Link href="/admin/assessments" className="hidden text-[0.58rem] font-semibold uppercase tracking-[0.14em] text-white/52 transition hover:text-white md:inline">Assessments</Link>
    ...
    <div className="print-hidden border-b border-black/10 bg-[#f7f5ef]"><div className="mx-auto flex max-w-[1560px] items-center justify-between px-5 py-3 sm:px-8 lg:px-10"><p className="text-xs text-black/48">Signed in as {account.displayName}</p><p className="text-[0.54rem] font-semibold uppercase tracking-[0.16em] text-[#80632d]">{roleLabel}</p></div></div>
    {children}
  </div>;
}
