...
export async function createEstimate(formData: FormData) {
  ...
  revalidatePath("/admin/billing/estimates");
  estimatePath(estimateNumber);
}
...
export async function sendEstimateEmail(formData: FormData) {
  ...
  if (!apiKey || !origin) estimatePath(estimateNumber, "email", "not-configured");
  ...
  if (!estimate) estimatePath(estimateNumber, "email", "not-found");
  ...
  if (!recipient) estimatePath(estimateNumber, "email", "missing-recipient");
  ...
  try {
    ...
    estimatePath(estimateNumber, "email", "failed");
  }
  if (!response.ok) estimatePath(estimateNumber, "email", "failed");
  ...
  estimatePath(estimateNumber, "email", "sent");
}
...
export async function convertEstimateToInvoice(formData: FormData) {
  ...
  const { data: estimate } = await supabase.from("estimates")
    ...
    .eq("estimate_number", estimateNumber).maybeSingle();
  if (!estimate || estimate.status !== "approved") estimatePath(estimateNumber, "convert", "not-approved");
  ...
  if (!items?.length) estimatePath(estimateNumber, "convert", "no-items");
  ...
  if (error || !invoice) estimatePath(estimateNumber, "convert", "failed");
  ...
  if (itemError) {
    ...
    estimatePath(estimateNumber, "convert", "failed");
  }
