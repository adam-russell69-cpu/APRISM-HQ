...
import { ArrowRight, BriefcaseBusiness, CreditCard, FileCheck2, FileWarning, ReceiptText, WalletCards } from "lucide-react";
...
export default async function AdminBillingPage() {
  ...
  const [accountsResult, locationsResult, workOrdersResult, estimatesResult, invoicesResult, paymentsResult] = await Promise.all([
    ...
    supabase.from("estimates").select("id, estimate_number, status, issue_date, valid_until, total, currency, client_accounts(display_name)").order("issue_date", { ascending: false }).limit(50),
  ...
  const dataError = [accountsResult, locationsResult, workOrdersResult, estimatesResult, invoicesResult, paymentsResult].some((result) => result.error);

  return <main className="mx-auto max-w-[1500px] px-4 py-7 sm:px-6 lg:px-8 lg:py-9">
    <AdminPageHeader eyebrow="Revenue operations" title="Client Billing" description="The internal record for estimates, invoices, outstanding balances, and verified payment status." actions={<Link href="/admin/billing/estimates/new" className="inline-flex min-h-10 items-center gap-2 bg-[#171a19] px-4 text-xs font-semibold text-white"><FileCheck2 className="size-4" />New Estimate</Link>} />
    ...
    <section className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
      {[
        ...
        { label: "Open estimates", value: estimates.filter((estimate) => ["draft", "sent"].includes(estimate.status)).length, detail: "Awaiting send or response", icon: FileCheck2 },
    ...
    <section className="mt-7"><div className="mb-3 flex items-end justify-between gap-4"><div><p className="text-[0.62rem] font-semibold uppercase tracking-[0.15em] text-[#88682f]">Pre-approval</p><h2 className="mt-1 font-serif text-3xl">Estimates</h2></div><Link href="/admin/billing/estimates/new" className="text-xs font-semibold text-[#765a29] hover:underline">Create estimate</Link></div>
      {estimates.length ? <AdminTable columns={["Estimate", "Client", "Dates", "Status", "Total"]}>{estimates.map((estimate) => { const account = firstRelation(estimate.client_accounts as Relation<{ display_name: string }>); return <AdminTableRow key={estimate.id} columns={5}><div><MobileLabel>Estimate</MobileLabel><Link href={`/admin/billing/estimates/${encodeURIComponent(estimate.estimate_number)}`} className="font-semibold text-[#765a29] hover:underline">{estimate.estimate_number}</Link></div><div><MobileLabel>Client</MobileLabel><p className="font-medium text-black/70">{account?.display_name ?? "Client account"}</p></div><div><MobileLabel>Dates</MobileLabel><p className="text-xs text-black/55">Issued {dateFormatter.format(new Date(`${estimate.issue_date}T12:00:00Z`))}</p><p className="mt-1 text-xs text-black/38">Valid to {dateFormatter.format(new Date(`${estimate.valid_until}T12:00:00Z`))}</p></div><div><MobileLabel>Status</MobileLabel><StatusBadge value={estimate.status} /></div><div className="flex items-center justify-between gap-3"><div><MobileLabel>Total</MobileLabel><p className="font-semibold">{formatMoney(Number(estimate.total), estimate.currency)}</p></div><ArrowRight aria-hidden="true" className="size-4 text-black/25" /></div></AdminTableRow>; })}</AdminTable> : <div className="border border-black/10 bg-white"><EmptyState icon={FileCheck2} title="No estimates" description="Create a client estimate before work requiring approval." /></div>}
    </section>
