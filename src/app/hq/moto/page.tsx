...
export default async function MotoPage(){
 ...
 const Card = ({ro}:{ro:(typeof rows)[number]}) => <Link href={`/hq/moto/ro/${ro.id}`} className="block rounded-2xl border border-[#b79a62]/25 bg-white/[0.035] p-4"><div className="flex justify-between gap-4"><div><p className="font-medium">{ro.service_name}</p><p className="mt-1 text-sm text-[#aaa398]">RO {ro.ro_number} · {ro.mileage_in.toLocaleString()} mi</p></div><div className="text-right"><span className="rounded-full border border-white/10 px-2 py-1 text-[10px] uppercase tracking-wider text-[#bdb7ac]">{ro.status.replaceAll("_", " ")}</span></div></div></Link>;

 return <div className="space-y-8">
  <section className="space-y-4"><div><p className="text-xs uppercase tracking-[0.22em] text-[#b79a62]">Outpost</p><h1 className="mt-2 text-3xl font-semibold">Moto</h1><p className="mt-2 text-sm text-[#aaa398]">Clients, repair orders and motorcycle history.</p></div><div className="grid grid-cols-2 gap-3"><Link href="/hq/moto/clients/new" className="rounded-xl border border-[#b79a62]/35 px-4 py-3 text-center text-sm font-semibold text-[#d7c08f]">+ New Client</Link><Link href="/hq/moto/ro/new" className="rounded-xl bg-[#c5aa72] px-4 py-3 text-center text-sm font-semibold text-[#151513]">+ New RO</Link></div><Link href="/hq/moto/clients" className="inline-block text-sm text-[#b79a62]">View Moto clients →</Link></section>
